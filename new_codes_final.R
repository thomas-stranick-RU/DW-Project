...
#write_csv(movies_final_sentiment, "C:/Users/polynLin/Desktop/mypart/movie_sentiment_scores_bing.csv")


#visualization
library(ggplot2)

# Select the top 15 movies with the most reviews for visualization
top_sentiment_movies <- movies_final_sentiment %>%
  filter(!is.na(sentiment_score)) %>%
  slice_max(order_by = sentiment_score, n = 15)

ggplot(top_sentiment_movies, aes(x = reorder(movie, sentiment_score), y = sentiment_score)) +
...
#The 15 least emotional movies
low_sentiment_movies <- movies_final_sentiment %>%
  filter(!is.na(sentiment_score)) %>%
  slice_min(order_by = sentiment_score, n = 15)

ggplot(low_sentiment_movies, aes(x = reorder(movie, sentiment_score), y = sentiment_score)) +
  ...
  theme_minimal()
# Top 15 by average sentiment value
top_avg_sentiment <- movies_final_sentiment %>%
  filter(!is.na(avg_sentiment)) %>%
  slice_max(order_by = avg_sentiment, n = 15)

ggplot(top_avg_sentiment, aes(x = reorder(movie, avg_sentiment), y = avg_sentiment)) +
  geom_col(fill = "darkgreen") +
  coord_flip() +
  labs(
    title = "Top 15 Movies by Average Sentiment",
    x = "Movie",
    y = "Average Sentiment (Score / Total Words)"
  ) +
  theme_minimal()

#Exploring whether sentiment scores can predict movie box office
...
ggplot(movies_final_model, aes(x = sentiment_score, y = log_gross)) +
  geom_point(color = "blue", alpha = 0.7) +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(
    title = "Sentiment Score vs Log(Gross)",
    x = "Sentiment Score",
    y = "Log(Gross Revenue)"
  ) +
  theme_minimal()

model_log <- lm(log_gross ~ sentiment_score, data = movies_final_model)
summary(model_log)
